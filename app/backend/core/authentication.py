...
from .cloudhelper import CloudConfiguration
...
class AuthenticationHelper:
    scope: str = f"{cloudConfig['microsoftGraphEndpoint']}/.default"
    ...
    def __init__(
        ...
        self.authority = f"{cloudConfig['entraIdAuthEndpoint']}/{tenant_id}"
    ...
    async def list_groups(graph_resource_access_token: dict) -> list[str]:
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            resp_json = None
            resp_status = None
            async with session.get(url=f"{cloudConfig['microsoftGraphEndpoint']}/v1.0/me/transitiveMemberOf?$select=id") as resp:
    ...
    async def get_auth_claims_if_enabled(self, headers: dict) -> dict[str, Any]:
        ...
        try:
            ...
            graph_resource_access_token = self.confidential_client.acquire_token_on_behalf_of(
                user_assertion=auth_token, scopes=[f"{cloudConfig['microsoftGraphEndpoint']}/.default"]
            )
