...
async def test_view_acl(monkeypatch, capsys):
    ...
    command = ManageAcl(
        ...
        index_name="INDEX",
        url=f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/content/a.txt",
        acl_action="view",
        acl_type="oids",
        acl="",
...
async def test_update_storage_urls(monkeypatch, caplog):
    ...
    command = ManageAcl(
        service_name="SERVICE",
        index_name="INDEX",
        url=f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/content/",
    ...
    with caplog.at_level(logging.INFO):
        await command.run()
        assert merged_documents == [{"id": 2, "storageUrl": f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/content/ab.txt"}]
        assert "Not updating storage URL of document 1 as it has only one oid and may be user uploaded" in caplog.text
        assert f"Adding storage URL https://test.blob.{cloudConfig['storageEndpointSuffix']}/content/ab.txt for document 2" in caplog.text
