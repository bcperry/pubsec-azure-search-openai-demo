...
def blob_manager(monkeypatch):
    return BlobManager(
        endpoint=f"https://{os.environ['AZURE_STORAGE_ACCOUNT']}.blob.{cloudConfig['storageEndpointSuffix']}",
    ...
    with NamedTemporaryFile(suffix=".pdf") as temp_file:
        ...
        async def mock_upload_blob(self, name, *args, **kwargs):
            ...
            return azure.storage.blob.aio.BlobClient.from_blob_url(
                f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/test/test.pdf", credential=MockAzureCredential()
            )

        monkeypatch.setattr("azure.storage.blob.aio.ContainerClient.upload_blob", mock_upload_blob)

        await blob_manager.upload_blob(f)
        assert f.url == f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/test/test.pdf"
...
async def test_upload_blob_no_image(monkeypatch, mock_env, caplog):
    ...
    with NamedTemporaryFile(suffix=".xlsx") as temp_file:
        ...
        async def mock_upload_blob(self, name, *args, **kwargs):
            assert name == filename
            return azure.storage.blob.aio.BlobClient.from_blob_url(
                f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/test/test.xlsx", credential=MockAzureCredential()
            )
        ...
        with caplog.at_level("INFO"):
            await blob_manager.upload_blob(f)
            assert f.url == f"https://test.blob.{cloudConfig['storageEndpointSuffix']}/test/test.xlsx"
